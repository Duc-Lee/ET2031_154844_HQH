...
back_img = pygame.transform.scale(back_img, (200, 82))  # kích thước
...
backgroundImg = pygame.transform.scale(backgroundImg, (1000, 600))  # Đặt kích thước ảnh nền
...
openHandImg = pygame.transform.scale(openHandImg, (160, 160))  # Thay đổi kích thước tay mở
...
closedHandImg = pygame.transform.scale(closedHandImg, (160, 160))  # Thay đổi kích thước tay đóng
closedHand_rect = closedHandImg.get_rect(topleft=(x, y))  # Vị trí tay đóng

# Tạo danh sách liên kết lưu thông tin về quả bóng
BallImg, BallX, BallY, Ball_rect, BallMoveX, BallMoveY = [], [], [], [], [], []
numberOfBalls = 7
...
for i in range(numberOfBombs):
    ...
    BombImg.append(pygame.image.load('bombb.png').convert_alpha())  # Tạo bom
    ...
    bombMoveX.append(3)  # Tốc độ di chuyển bom theo chiều X
    bombMoveY.append(3)  # Tốc độ di chuyển bom theo chiều Y

# Game Texts
score_value = 0   # Biến đếm điểm khi bắt được trái bóng
...
while True:
    ...
    if not game_over:  # Chỉ tiếp tục các hoạt động này nếu trò chơi chưa kết thúc
        ...
        if hands:
            ...
            openHand_rect.left = width - ((positionOfTheHand[9][0] - 100) * 1.9)
            openHand_rect.top = (positionOfTheHand[9][1] - 100) * 1.9
            closedHand_rect.left = width - ((positionOfTheHand[9][0] - 100) * 1.9)
            closedHand_rect.top = (positionOfTheHand[9][1] - 100) * 1.9
            # open or closed hand
                ...
                if fingers[0] * fingers[1] * fingers[2] * fingers[3]:
                    ...
                    for i in range(numberOfBombs):
                        if openHand_rect.colliderect(bomb_rect[i]):
                            bombs_caught += 1
                            bomb_rect[i] = BombImg[i].get_rect(
                                topleft=(random.randint(0, width), random.randint(0, height)))
                            if bombs_caught == 3:
                                game_over = True
                    catch_insect_with_openHand = False
                    ...
                    for iterate in range(numberOfBombs):
                        if openHand_rect.colliderect(bomb_rect[iterate]):
                            catch_insect_with_openHand = True
        # Opencv Screen
